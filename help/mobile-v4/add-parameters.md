...
[!UICONTROL 설정] > [!UICONTROL 속성] 아래의 [!DNL Target] 인터페이스에서 at_property 값을 검색할 수 있습니다.  속성 위로 마우스를 가져간 후 코드 조각 아이콘을 선택하고 `at_property` 값을 복사합니다.
...
다음은 `targetPrefetchContent()` 함수에 대한 업데이트된 코드입니다(_[!UICONTROL at_property 값이 여기에 있음]_ 자리 표시자 텍스트를 업데이트하십시오!).
