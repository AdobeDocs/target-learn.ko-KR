...
* 자신의 계정에 요청할 수 있도록 Adobe Target 클라이언트 코드를 알고 있습니다. 클라이언트 코드는 Adobe Target 인터페이스의 설정 > 구현 > at.js 설정 편집 화면에 표시됩니다
...
1. 아래 그림과 같이 Adobe Target의 활동 목록에 도달하면 사용자에게 승인자 수준 액세스 권한이 있는지 확인해야 합니다. [!DNL Target]에 액세스할 수 없거나 승인자 수준 액세스 권한을 확인할 수 없는 경우 회사의 Experience Cloud 관리자에게 연락하여 이 액세스 권한을 요청하십시오. 권한이 부여되면 이 자습서를 다시 시작하십시오.
