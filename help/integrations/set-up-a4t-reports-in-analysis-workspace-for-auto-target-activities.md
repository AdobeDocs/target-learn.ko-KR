...
>[!IMPORTANT]
>
>[!UICONTROL 자동 타겟] 활동의 경우 [!DNL Analytics Workspace]에서 보고를 확인하고 A4T 패널을 수동으로 만들어야 합니다.
...
[!DNL Adobe Analytics] [!DNL Analysis Workspace]에서 다양한 분석 기능을 사용할 수 있지만 실험 활동(수동 [!UICONTROL A/B 테스트] 및 [!UICONTROL 자동 할당])과 개인화 활동([!UICONTROL [!UICONTROL 자동 타겟]]) 간의 차이로 인해 [!DNL Auto-Target] 활동을 올바르게 해석하려면 기본 **[!UICONTROL Analytics for Target]** 패널을 일부 수정해야 합니다.
...
[!UICONTROL 자동 타겟] 보고서에 대한 A4T를 만들려면 아래와 같이 [!DNL Analysis Workspace]의 **[!UICONTROL Analytics for Target]** 패널로 시작하거나 자유 형식 테이블로 시작하십시오. 그런 다음 다음을 선택합니다.

1. **[!UICONTROL 경험 제어]**: 모든 경험을 선택할 수 있지만 나중에 이 선택을 재정의합니다. [!UICONTROL 자동 타겟] 활동의 경우 제어 경험은 실제로 제어 전략이며, a) 모든 경험에서 임의로 제공되거나 b) 단일 경험을 제공합니다(이 선택은 [!DNL Adobe Target]의 활동 생성 시 수행됩니다). 선택(b)을 선택하더라도 [!UICONTROL 자동 타겟] 활동에서 특정 경험을 제어로 지정했습니다. [!UICONTROL 자동 타겟] 활동에 대한 A4T 분석을 위해 이 자습서에 설명된 접근 방식을 따라야 합니다.
2. **[!UICONTROL 지표 정규화]**: [!UICONTROL 방문]을 선택하십시오.
3. **[!UICONTROL 성공 지표]**: 보고할 지표를 선택할 수는 있지만 일반적으로 [!DNL Target]에서 활동을 만드는 동안 최적화를 위해 선택한 지표와 동일한 지표에 대한 보고서를 확인해야 합니다.

   [!UICONTROL 자동 타겟] 활동에 대한 ![[!UICONTROL Analytics for Target] 패널 설정.](assets/Figure1.png)

   *그림 1: [!UICONTROL 자동 타겟] 활동에 대한 [!UICONTROL Analytics for Target] 패널 설정.*

>[!TIP]
>
>[!UICONTROL 자동 타겟] 활동에 대해 [!UICONTROL Analytics for Target] 패널을 설정하려면 모든 제어 경험을 선택하고 [!UICONTROL 방문]을 표준화 지표로 선택한 다음 [!DNL Target] 활동을 만드는 동안 최적화를 위해 선택한 것과 동일한 목표 지표를 선택하십시오.

## [!UICONTROL 컨트롤 및[!DNL Target] 앙상블 ML 모델을 제어와 비교하기 위해 타깃팅된] 차원

기본 A4T 패널은 클래식(수동) [!UICONTROL A/B 테스트] 또는 [!UICONTROL 자동 할당] 활동을 위해 설계되었습니다. 여기서 목표는 개별 경험의 성능을 제어 경험과 비교하는 것입니다. 그러나 [!UICONTROL 자동 타겟] 활동에서 첫 번째 순서는 제어 *전략*&#x200B;과(와) 타깃팅된 *전략* 간의 비교여야 합니다. 즉, 제어 전략에 대한 [!UICONTROL 자동 타겟] 앙상블 ML 모델의 전체 성능 상승도를 결정합니다.

이 비교를 수행하려면 **[!UICONTROL 제어와 타깃팅(Analytics for Target)]** 차원을 사용하십시오. 끌어다 놓아 기본 A4T 보고서에서 **[!UICONTROL Target 경험]** 차원을 바꾸십시오.
...
>[!TIP]
>
>[!DNL Analysis Workspace]에서 톱니바퀴 아이콘을 클릭하여 [!UICONTROL 전환율] 열의 백분율을 숨겨 경험 전환율에 초점을 맞추십시오. 그러면 전환율은 소수로 포맷되지만 그에 따라 백분율로 해석됩니다.

## [!UICONTROL 자동 타겟] 활동에 대해 &quot;[!UICONTROL 방문 횟수]&quot;이(가) 올바른 표준화 지표인 이유

[!UICONTROL 자동 타겟] 활동을 분석할 때 항상 [!UICONTROL 방문 횟수]를 기본 표준화 지표로 선택하십시오. [!UICONTROL 자동 타겟] 개인화는 방문당 한 번(공식적으로 [!DNL Target] 세션당 한 번) 방문자에 대한 경험을 선택합니다. 즉, 방문자에게 표시되는 경험이 모든 방문 시 변경될 수 있습니다. 따라서 표준화 지표로 [!UICONTROL 고유 방문자 수]를 사용하는 경우 한 명의 사용자에게 서로 다른 방문에 걸쳐 여러 경험이 표시될 수 있으므로 전환율이 혼동될 수 있습니다.
...
사용자가 [!UICONTROL 자동 타겟] 활동과 실제로 상호 작용한 방문에 대해 보고하려는 경우(활동 시작, 디스플레이 또는 방문 이벤트 또는 전환을 통해) 다음을 수행할 수 있습니다.

1. 해당 [!DNL Target] 활동의 히트를 포함하는 특정 세그먼트를 만든 다음
1. 이 세그먼트를 사용하여 [!UICONTROL 방문] 지표를 필터링합니다.

**세그먼트를 만들려면:**

1. [!DNL Analysis Workspace] 도구 모음에서 **[!UICONTROL 구성 요소 > 세그먼트 만들기]** 옵션을 선택합니다.
2. 세그먼트에 대한 **[!UICONTROL 제목]**&#x200B;을 지정하십시오. 아래 예제에서 세그먼트 이름은 [!DNL "Hit with specific Auto-Target activity"]입니다.
3. **[!UICONTROL Target 활동]** 차원을 세그먼트 **[!UICONTROL 정의]** 섹션으로 끌어옵니다.
...
세그먼트가 만들어지면 이 세그먼트를 사용하여 [!UICONTROL 방문] 지표를 필터링하면 [!UICONTROL 방문] 지표에는 사용자가 [!DNL Target] 활동과 상호 작용한 방문만 포함됩니다.

**이 세그먼트를 사용하여 [!UICONTROL 방문]을 필터링하려면:**

1. 새로 만든 세그먼트를 구성 요소 도구 모음에서 드래그하고 파란색 **[!UICONTROL 필터링 기준]** 프롬프트가 나타날 때까지 **[!UICONTROL 방문]** 지표 레이블의 아래쪽을 가리키십시오.
...
A4T 통합을 통해 [!DNL Adobe Analytics]이(가) *성능 보고서를 생성*&#x200B;하는 데 사용하는 것과 동일한 전환 이벤트 데이터를 사용하여 [!UICONTROL 자동 타겟] ML 모델을 *훈련*&#x200B;할 수 있습니다. 그러나 ML 모델을 교육할 때 이 데이터를 해석하는 데 사용해야 하는 특정 가정이 있으며, 이는 [!DNL Adobe Analytics]의 보고 단계 동안 수행되는 기본 가정과 다릅니다.
...
1. 결과 메뉴에서 **[!UICONTROL 데이터 설정]**(으)로 스크롤합니다.
1. **[!UICONTROL 기본값이 아닌 속성 모델 사용]**&#x200B;을 선택합니다(아직 선택하지 않은 경우).

   ![비defaultattributionmodel.png](assets/non-defaultattributionmodel.png)

1. **[!UICONTROL 편집]**&#x200B;을 클릭합니다.
1. **[!UICONTROL 모델]**: **[!UICONTROL 기여도]** 및 **[!UICONTROL 전환 확인 기간]**: **[!UICONTROL 방문]**&#x200B;을 선택합니다.
...
1. 이전과 같이 [!DNL Analysis Workspace] 도구 모음에서 **[!UICONTROL 구성 요소 > 세그먼트 만들기]** 옵션을 선택하십시오.
2. 세그먼트에 대한 **[!UICONTROL 제목]**&#x200B;을 지정하십시오.
...
4. 세그먼트 정의 컨테이너의 왼쪽 상단에서 **[!UICONTROL 포함]** **방문**&#x200B;을 선택합니다.
5. **[!UICONTROL 은(는)]**&#x200B;보다 큼 연산자를 사용하고 값을 0으로 설정하십시오.
...
이전 섹션에서 [!UICONTROL 방문] 및 목표 지표를 수정하면 [!DNL Auto-Target] 보고 패널의 기본 A4T에 대해 마지막으로 수정해야 하는 수정 사항은 적절하게 필터링된 &quot;방문&quot; 지표에 대한 수정된 목표 지표의 올바른 비율인 전환율을 만드는 것입니다.

다음 단계를 사용하여 [!UICONTROL 계산된 지표]을(를) 만들어 이 작업을 수행합니다.

1. [!DNL Analysis Workspace] 도구 모음에서 **[!UICONTROL 구성 요소 > 지표 만들기]** 옵션을 선택합니다.
1. 지표에 대해 **[!UICONTROL 제목]**&#x200B;을 지정하십시오. 예를 들어 &quot;활동 XXX에 대한 방문 수정 전환율&quot;이 있습니다.
1. **[!UICONTROL 형식]** = 백분율 및 **[!UICONTROL 소수점 이하 자리 수]** = 2를 선택합니다.
1. 활동에 대한 관련 목표 지표(예: [!UICONTROL 활동 전환])를 정의로 드래그하고 이 목표 지표의 톱니바퀴 아이콘을 사용하여 앞에서 설명한 대로 기여도 분석 모델을 (방문)으로 조정합니다.
1. **[!UICONTROL 정의]** 섹션의 오른쪽 상단에서 **[!UICONTROL 추가 > 컨테이너]**&#x200B;를 선택합니다.
1. 두 컨테이너 간÷ 나눗셈(나눗셈) 연산자를 선택합니다.
1. 이 특정 [!DNL Auto-Target] 활동에 대해 이 자습서에서 &quot;특정 [!UICONTROL 자동 타겟] 활동으로 히트&quot;라는 이전에 만든 세그먼트를 드래그합니다.
...
>[!IMPORTANT]
>
>A4T 패널의 [!UICONTROL 전환] 속도 지표가 테이블의 전환 이벤트 또는 표준화 지표에 연결되어 있지 않습니다. 이 자습서에서 제안한 내용을 수정하면 [!UICONTROL 전환] 속도가 변경 내용에 자동으로 적용되지 않습니다. 따라서 전환 이벤트 속성 또는 표준화 지표(또는 둘 다)를 수정하는 경우 위와 같이 [!UICONTROL 전환] 비율도 수정하는 마지막 단계로 기억해야 합니다.

## 요약: [!UICONTROL 자동 타겟] 보고서에 대한 최종 샘플 [!DNL Analysis Workspace] 패널

위의 모든 단계를 단일 패널로 결합하면 아래 그림에서 [!UICONTROL 자동 타겟] A4T 활동에 대한 권장 보고서를 전체적으로 볼 수 있습니다. 이 보고서는 [!DNL Target] ML 모델에서 목표 지표를 최적화하는 데 사용한 보고서와 동일합니다. 이 보고서에는 이 자습서에서 설명한 모든 뉘앙스와 권장 사항이 통합되어 있습니다. 이 보고서는 기존의 [!DNL Target] 보고 기반 [!UICONTROL 자동 타겟] 활동에서 사용되는 계산 방법론과도 가장 가깝습니다.

이미지를 확장하려면 클릭하십시오.

![Analysis Workspace의 [!DNL Analysis Workspace]](assets/Figure10.png "A4T 보고서에서 최종 A4T 보고서"){width="600" zoomable="yes"}

*그림 10: 이 자습서의 이전 섹션에 설명된 지표 정의에 대한 모든 조정을 결합하는 [!DNL Adobe Analytics] [!DNL Workspace]의 최종 A4T [!UICONTROL 자동 타겟] 보고서.*
