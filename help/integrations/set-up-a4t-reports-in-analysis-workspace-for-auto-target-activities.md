...
이 문서에서는 다음 주요 개념을 기반으로 하는 작업 공간에서 [!DNL Auto-Target] 활동을 분석하기 위한 권장 수정 사항을 안내합니다.
...
* 정해진 속성 모델에서 &quot;방문 룩백(visit lookback) 윈도우&quot;라고도 하는 Visit-lookback 범위 속성은 교육 단계 동안 Adobe Target의 ML 모델에서 사용되며, 목표 지표를 분류할 때 동일한(기본값이 아닌) 속성 모델을 사용하는 것이 가장 좋습니다.

## 작업 공간에서 [!DNL Auto-Target] 패널에 대한 A4T 만들기

[!DNL Auto-Target] 보고서에 대한 A4T를 만들려면 아래와 같이 작업 공간의 Target 패널에 대한 일반적인 분석으로 시작하거나 자유 형식 테이블로 시작합니다. 그런 다음 다음을 선택합니다.

1. **[!UICONTROL 제어 경험]**:어떤 경험이든 선택할 수 있습니다.그러나 나중에 이 선택을 무시합니다. [!DNL Auto-Target] 활동의 경우 제어 경험은 (a) 모든 경험들 사이에 임의로 제공되거나, b) 단일 경험을 제공하는 제어 전략입니다(이 선택 사항은 Adobe Target에서 활동 생성 시 수행됨). 선택 (b) - [!DNL Auto-Target] 활동이 특정 경험을 제어로 지정한 경우에도 [!DNL Auto-Target] 활동에 대한 A4T 분석을 위해 이 문서에 설명된 접근 방식을 따라야 합니다.
1. **[!UICONTROL 표준화 지표]**:방문을 선택합니다.
1. **[!UICONTROL 성공 지표]**:보고할 지표를 선택할 수 있지만 일반적으로 Adobe Target에서 활동을 생성하는 동안 최적화를 위해 선택된 동일한 지표에 대한 보고서를 봐야 합니다.
...
## Adobe Target의 앙상블 ML 모델을 제어와 비교하기 위해 제어 및 타깃팅된 차원을 사용합니다.
...
이 교체는 A4T 패널의 기본 리프트 및 신뢰도 계산을 무효화한다는 점을 참고하십시오. 혼동을 방지하기 위해 다음 보고서를 남겨둔 채 기본 패널에서 이러한 지표를 제거할 수 있습니다.

![그림 2.png](assets/Figure2.png)
*그림 2:활동에 대한 권장 기준  [!DNL Auto-Target] 보고서입니다. 이 보고서는 타깃팅된 트래픽(앙상블 ML 모델에서 제공)과 제어 트래픽*&#x200B;을(를) 비교하도록 구성되었습니다.
...
[!DNL Auto-Target] 활동을 분석할 때는 항상 방문 수를 기본 표준화 지표로 선택합니다. [!DNL Auto-Target] 개인화는 방문당 한 번(공식적으로, Adobe Target 세션당 한 번)의 방문자에 대한 경험을 선택합니다. 즉, 사용자에게 표시되는 경험은 방문마다 변경할 수 있습니다. 따라서 고유 방문자를 표준화 지표로 사용한 경우 단일 사용자가 여러 경험(다른 방문에 걸쳐)을 보게 되어 전환율이 혼동될 수 있습니다.
...
표에 표시된 것처럼 방문자 수준 수치에는 명확한 연관성이 없습니다. 총 고유 방문자 수가 2명이라는 사실에도 불구하고 각 경험에 대한 개별 고유 방문자 수의 합은 아닙니다. 방문자 수준 전환율이 반드시 잘못된 것은 아니지만, 한 사람이 개별 경험을 비교할 때 방문 수준의 전환율은 거의 틀림없이 더 이치에 가까운 결과를 가져옵니다. 일반적으로 분석 단위(&quot;방문 횟수&quot;)는 의사 결정 고착성 단위와 동일하며, 이는 측정 지표의 경험 수준 분류의 합계와 비교가 적절함을 의미합니다.
...
1. 세그먼트의 **[!UICONTROL 제목]**&#x200B;을 입력합니다. 아래 예에서 세그먼트 이름은 [!DNL "Hit with specific Auto-Target activity"]입니다.
1. **[!UICONTROL Target 활동]** 차원을 세그먼트 **[!UICONTROL 정의]** 섹션으로 드래그합니다.
1. **[!UICONTROL equals]** 연산자를 사용합니다.
1. 특정 Target 활동을 검색합니다.
1. 톱니바퀴 아이콘을 선택하고 아래 그림과 같이 **[!UICONTROL 속성 모델 > 인스턴스]**&#x200B;를 선택합니다.
1. **[!UICONTROL 저장]**&#x200B;을 클릭합니다.

![그림 5.png](assets/Figure5.png)
*그림 5:여기에 표시된 세그먼트와 같은 세그먼트를 사용하여 보고서에 대한 A4T의 방문 횟수 지표를  [!DNL Auto-Target] 필터링합니다*

세그먼트가 만들어지면 사용자가 Target 활동과 상호 작용한 방문만 포함하도록 방문 지표를 필터링하는 데 사용할 수 있습니다.
...
1. 세그먼트를 해제합니다. 필터가 해당 지표에 적용됩니다.
...
## 고급:목표 지표에 대한 방문 범위 속성

A4T 통합을 통해 Adobe Analytics에서 성능 보고서 생성&#x200B;*에 사용된 것과 동일한 전환 이벤트 데이터를 사용하여 [!DNL Auto-Target]의 ML 모델을*&#x200B;교육&#x200B;*할 수 있습니다.* 그러나 ML 모델을 교육할 때 이 데이터를 해석하는 데 사용해야 하는 가정도 있습니다. XML 모델은 Adobe Analytics에서 보고 단계 동안 수행된 기본 가정과는 다릅니다.
...
## 최종 단계:방문 범위 속성 및 필터링된 방문 표준화를 통한 전환율
...
1. 이 특정 [!DNL Auto-Target] 활동에 대해 이 문서에서 &quot;특정 [!DNL Auto-Target] 활동을 사용하여 히트&quot;라는 이전에 만든 세그먼트를 드래그합니다.
...
>[!IMPORTANT]
>
>A4T 패널의 전환율 지표는 표의 전환 이벤트 또는 표준화 지표에 연결되어 있지 않습니다. 이 문서에서 제안된 수정 사항을 만들 때 전환율은 변경 사항에 적용되지 않습니다. 따라서 전환 이벤트 속성 및 표준화 지표 중 하나(또는 둘 다)로 수정하는 경우 위에 표시된 대로 전환율을 수정하기 위한 마지막 단계로 기억해야 합니다.

## 요약:[!DNL Auto-Target] 보고서에 대한 최종 샘플 작업 공간 패널

위의 모든 단계를 하나의 패널에 결합하여 아래 그림에서는 [!DNL Auto-Target] A4T 활동에 대한 권장 보고서의 전체 보기를 보여 줍니다. 이 보고서는 목표 지표를 최적화하기 위해 Target의 기계 학습 모델에서 사용하는 보고서와 동일하며 이 문서에서 설명한 모든 뉘앙스와 권장 사항을 통합합니다. 이 보고서는 기존의 Target 보고 기반 [!DNL Auto-Target] 활동에서 사용되는 계산 방법과도 가장 가깝습니다.
